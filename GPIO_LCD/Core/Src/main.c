...
void LCD_Write_Byte(uint8_t data,uint8_t mode);// write 1 byte
...
void LCD_Init(){
	HAL_Delay(50);
	LCD_Write_cmd(0x38); //Function set 8bit interface, 2line 5*7 pixels
	HAL_Delay(5);
	LCD_Write_cmd(0x0C);
	HAL_Delay(5);
	LCD_Write_cmd(0x06); //kieu hien chu tu trai sang phai
	HAL_Delay(5);
}
